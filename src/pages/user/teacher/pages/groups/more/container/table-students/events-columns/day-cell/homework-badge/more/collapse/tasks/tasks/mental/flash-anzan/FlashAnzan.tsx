    ...
    return (
        ...
        >
            {totals.map((total: any, keyExercise: number) => (
                <tr key={keyExercise}>
                    <td className="number">{keyExercise + 1}</td>
                    <td
                        className={`trophy ${
                            total.result ? "warning" : "second"
                        }`}
                    >
                        <TrophyOutlined />
                    </td>
                    <td className="answer">{total.answer}</td>
                    <td
                        className={`user ${
                            total.result ? "success" : "danger"
                        }`}
                    >
                        {total.user !== undefined ? total.user : "Пусто"}
                    </td>
                    <td className="exercises">{total.exercise}</td>
                </tr>
            ))}
        </ResultMoreLayout>
    )
